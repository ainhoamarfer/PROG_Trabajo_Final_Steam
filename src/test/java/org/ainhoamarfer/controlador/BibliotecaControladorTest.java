...
//
    //@Mock
    //private IUsuarioRepo usuarioRepo;   // por si más adelante se valida el usuario
//
    //@InjectMocks
...
//
    ...
    //            idBiblioteca, idUsuario, usuario, idJuego, juego,
    //            fecha, 0.0, null, false
    //    );
    //}
//
    //// -----------------------------------------------------------------
    //// Tests para verBibliotecaPersonal
    //// -----------------------------------------------------------------
//
    //@Test
...
//
    ...
    //        mockedMapper.when(() -> Mapper.mapDeBiblioteca(any()))
    //                .thenAnswer(inv -> {
    //                    BibliotecaEntidad b = inv.getArgument(0);
    //                    return new BibliotecaDTO(b.getId(), b.getUsuarioId(), null, b.getJuegoId(), null,
    //                            b.getFechaAdquisicion(), b.getTiempoJuego(), b.getFechaUltimaJugado(), b.isInstalado());
    //                });
//
    //        List<BibliotecaDTO> resultado = controlador.verBibliotecaPersonal(idUsuario, null);
//
...
//
    //        assertEquals(2, resultado.size());
    //        assertEquals(fecha, resultado.get(0).getFechaAdquisicion());       // más reciente primero
    //        assertEquals(fecha.minusDays(10), resultado.get(1).getFechaAdquisicion());
    //    }
    //}
//
// -//----------------------------------------------------------------
// T//ests para eliminarJuegoBiblioteca
// -//----------------------------------------------------------------
//
    //@Test
...
//
    //    try (MockedStatic<Mapper> mockedMapper = mockStatic(Mapper.class)) {
    //        mockedMapper.when(() -> Mapper.mapDeBiblioteca(bibliotecaNueva)).thenReturn(bibliotecaDTO);
//
...
//
// -//----------------------------------------------------------------
// T//ests para actualizarTiempoJuego
// -//----------------------------------------------------------------
//
    //@Test
...
//
    //    try (MockedStatic<Mapper> mockedMapper = mockStatic(Mapper.class)) {
    //        mockedMapper.when(() -> Mapper.mapDeBiblioteca(biblioteca)).thenReturn(bibliotecaDTO);
//
...
//
// -//----------------------------------------------------------------
// T//ests para consultarUltimaSesion
// -//----------------------------------------------------------------
//
    //@Test
...
//
    //        assertNotNull(resultado);
    //        // Verificar que el DTO tiene la fecha esperada
...
//
    ...
    //        assertEquals(idJuego, resultado.getIdJuego());
    //        verify(biblioRepo).crear(any(BibliotecaForm.class));
    //    }
    //}
//
    //@Test
    //void anadirJuegoBiblioteca_CuandoUsuarioNoTieneNingunaBiblioteca_DeberiaCrearCorrectamente() throws ExcepcionValidacion {
    //    when(biblioRepo.obtenerPorIdUsuario(idUsuario)).thenReturn(List.of());
    //    when(juegoRepo.obtenerPorId(idJuego)).thenReturn(Optional.of(juego));
    //    when(biblioRepo.crear(any(BibliotecaForm.class))).thenReturn(Optional.of(bibliotecaNueva));
//
